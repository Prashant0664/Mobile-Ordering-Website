...
const Getdata = async () => {
  ...
  try {
    const data = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/sales`
    );
...
const GetdatatopSales = async () => {
  ...
  try {
    const data = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/topratedsales`
    );
    for (var i = 0; i < data.data.length; i++) {
      var jsnfile = data.data[i].jsonfile;
      jsnfile = JSON.parse(jsnfile);
...
const Heroapi = async () => {
  ...
  try {
    const clips = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/clips`
    );
...
const Highlight = async () => {
  try {
    const clips = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/highlights`
    );
